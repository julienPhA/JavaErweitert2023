...
public class OfenController {
	...
	public HashMap erzeugeBrennelementlisteVonDB(String material) {
		...
		try {		
				...
				String select = "SELECT typ,class,bezeichnung,brennwert FROM brennelemente WHERE typ = ?";
	...
	public void verbrenneSortiert(Message m) {
		//HashMap<String,String> brennelementeListe = erzeugeBrennelementliste();
		String[] s = m.toString().split("=");
		String[] material=s[1].split("]");
						
		System.out.println("übergebenes Material: " + material[0]);
		
		HashMap<String,String> brennelementeListe = erzeugeBrennelementlisteVonDB(material[0]);
